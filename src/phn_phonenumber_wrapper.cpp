...
const static mcc_cc_map_s _mcc_cc_list[] = {
	{0, 1},
	{202, 30},
	{204, 31},
	{206, 32},
...
static TapiHandle *_tapi_handle_sim2 = NULL;
...
static void _phn_cc_changed_cb(
		TapiHandle *handle, const char *noti_id, void *data, void *user_data)
{
	int ret;
...
static int _phn_get_tapi_handle()
{
	...
	RETVM_IF(NULL == cp_list, PHONE_NUMBER_ERROR_SYSTEM, "tel_get_cp_name_list() Fail(NULL)");

	while(cp_list[_modem_num])
		_modem_num++;
	DBG("_modem_num=[%d]", _modem_num);

	_tapi_handle = tel_init(cp_list[0]);
	...
	ret = tel_register_noti_event(_tapi_handle, TAPI_PROP_NETWORK_PLMN,
	...
	if (2 == _modem_num) {
		_tapi_handle_sim2 = tel_init(cp_list[1]);
		if (NULL == _tapi_handle_sim2) {
			ERR("tel_init() for SIM2 Fail");
			g_strfreev(cp_list);
			return PHONE_NUMBER_ERROR_SYSTEM;
		}

		ret = tel_register_noti_event(_tapi_handle_sim2, TAPI_PROP_NETWORK_PLMN,
				_phn_cc_changed_cb, NULL);
		if (TAPI_API_SUCCESS != ret) {
			ERR("tel_register_noti_event() for SIM2 Fail(%d)", ret);
			g_strfreev(cp_list);
			return PHONE_NUMBER_ERROR_SYSTEM;
		}
	}
...
static int _phn_get_cc(bool reload, int *out_cc)
{
	...
	if (NULL == _tapi_handle) {
		...
		RETVM_IF(PHONE_NUMBER_ERROR_NONE != ret, ret,
				"_phn_get_tapi_handle() Fail(%d)", ret);
	}

	ret = tel_get_property_int(_tapi_handle, TAPI_PROP_NETWORK_SERVICE_TYPE, &state);
	if (TAPI_API_SUCCESS != ret) {
	...
	ret = tel_get_property_string(_tapi_handle, TAPI_PROP_NETWORK_PLMN, &temp);
	...
	if (NULL == temp || '\0' == temp[0]) {
		if (2 == _modem_num) {
			...
			ret = tel_get_property_int(_tapi_handle_sim2, TAPI_PROP_NETWORK_SERVICE_TYPE, &state);
			if (TAPI_API_SUCCESS != ret) {
				ERR("tel_get_property_int() Fail(%d)", ret);
				return PHONE_NUMBER_ERROR_SYSTEM;
			}
			...
			ret = tel_get_property_string(_tapi_handle_sim2, TAPI_PROP_NETWORK_PLMN, &temp);
			if (ret != TAPI_API_SUCCESS) {
				ERR("tel_get_property_string() Fail(%d)", ret);
				return PHONE_NUMBER_ERROR_SYSTEM;
			}
			DBG("temp=[%s] from SIM2", temp);
		}
	}
